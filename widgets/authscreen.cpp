	...
	ui(new Ui::AuthScreen)
{
	...
	db::PSQL::getInstance()->set(&query);
	query = "create table auth (id uuid default uuid_generate_v4() primary key, password text not null, emailid uuid references emails(id), active boolean default true not null)";
	db::PSQL::getInstance()->set(&query);
